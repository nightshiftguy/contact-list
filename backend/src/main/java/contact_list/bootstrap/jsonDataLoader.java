package contact_list.bootstrap;

import contact_list.contact.ContactRepository;
import contact_list.contact.Contacts;
import contact_list.user.UserRepository;
import contact_list.user.Users;
  ...
  public void run(String... args) throws Exception {
    if (userRepository.count() == 0) {
      try (InputStream inputStream = TypeReference.class.getResourceAsStream("/data/users.json")) {
        ...
        log.info(
            "Reading {} users form JSON data and saving to local database",
            allUsers.users().size());
    ...
    if (contactRepository.count() == 0) {
      try (InputStream inputStream =
          TypeReference.class.getResourceAsStream("/data/contacts.json")) {
        Contacts allContacts = objectMapper.readValue(inputStream, Contacts.class);
        log.info(
            "Reading {} contacts form JSON data and saving to local database",
            allContacts.contacts().size());
